...
const getOTPEmailTemplate = (userName, otpCode) => {
  return {
    subject: `🔐 ${otpCode} is your HASA GOLD verification code`,
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Verification Code - HASA GOLD STORE</title>
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; background: #f4f4f4; }
          .container { max-width: 600px; margin: 20px auto; background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
          .header { background: linear-gradient(135deg, #0A0A0B, #1A1A1B); padding: 40px 30px; text-align: center; border-bottom: 3px solid #FFD700; }
          .logo { font-size: 28px; font-weight: 900; color: #FFD700; letter-spacing: 2px; }
          .content { padding: 40px 30px; text-align: center; }
          .greeting { font-size: 22px; font-weight: 700; color: #333; margin-bottom: 15px; }
          .message { font-size: 16px; color: #666; margin-bottom: 30px; }
          .otp-container { background: #f8f9fa; border: 2px dashed #FFD700; border-radius: 12px; padding: 25px; margin: 30px 0; }
          .otp-code { font-size: 42px; font-weight: 900; color: #000; letter-spacing: 10px; font-family: monospace; }
          .expiry { font-size: 12px; color: #999; margin-top: 10px; }
          .footer { background: #f8f9fa; color: #999; padding: 20px; text-align: center; font-size: 12px; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p class="message">Hello ${userName}, use the code below to verify your email address and complete your registration.</p>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'verification' && verificationLink) {
      ...
      emailTemplate = getOTPEmailTemplate(userName || 'Gamer', otpCode);
